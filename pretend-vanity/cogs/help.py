...
class HelpSelect(Select):
    def __init__(self, bot):
        options = [
            discord.SelectOption(label="Settings", description="Settings commands", value="settings"),
            discord.SelectOption(label="Utilities", description="Utility commands", value="utilities"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        commands = {
            "settings": [
                ("set keeprole <bool_value>", "Sets the keep role option for the guild."),
                ("set vanity <vanity>", "Sets the vanity for the guild."),
                ("set notifications <bool_value>", "Toggles the notifications for the guild."),
                ("set vanitylock <bool_value>", "Sets the vanity lock for the guild."),
                ("set channel <notif_channel>", "Sets the notification channel for the guild."),
                ("set role <role>", "Sets the role to be assigned for the guild."),
            ],
            "utilities": [
                ("settings", "Displays the current settings for the guild."),
                ("stats", "Displays the number of people who have the vanity in their status.")
            ]
        }
...
class CustomHelp(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        """
        embed = Embed(
            description="Please select a category to view commands.",
            color=0x729BB0
        )
